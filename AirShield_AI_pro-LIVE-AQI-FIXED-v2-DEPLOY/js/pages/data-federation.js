...
ROUTES.data = function(v){
  const steps = [
    ...
    ['Local sensors', 'Low-cost PM monitors across the ward grid', 'demo', 'sensor'],
    ['Satellite data', 'Aerosol optical depth tiles, resampled to 1 km', 'demo', 'target'],
    ['Meteorological data', 'Wind, humidity, temperature and boundary layer height', 'live', 'wind'],
  ...
  v.innerHTML =
    ...
    '<div class="split">' +
      '<div class="card"><div class="card-head"><h2>'+esc(t('pipeline'))+'</h2>' +
        ...
        '</div>' +
        '<div class="banner" style="margin-top:14px">'+ICON.info+'<div>The meteorological step is the only one wired for a real API. With no key present, AirShield falls back to demo values — see Settings.</div></div>' +
      '</div>' +
