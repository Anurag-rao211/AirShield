...
ROUTES.settings = function(v){
  v.innerHTML =
    ...
    '<div class="grid g-2">' +
      ...
      '</div>' +
      '<div class="card"><div class="card-head"><h2>'+esc(t('dataModeL'))+' & API Integration</h2></div>' +
        ...
        '<div class="banner">'+ICON.info+'<div id="apiNoteText">'+esc('Live AQI is fetched server-side through Vercel. The WAQI token is secured environment-side and never exposed to the browser.')+'</div></div>'+'<div style="margin-top:10px;display:flex;align-items:center;gap:10px;flex-wrap:wrap"><button class="btn btn-sm" id="checkLive">Check live service</button><span id="liveDiag" class="tiny muted"></span></div>' +
  ...
  async function runDiag(){
    const b=$('#checkLive'),o=$('#liveDiag'); if (!b || !o) return;
    b.disabled=true; o.textContent='Checking…';
    const s=await checkLiveService();
    o.textContent=s.ok?'✓ WAQI connected and returning live data.':(s.tokenConfigured?(s.error||'WAQI is not reachable or returned no live station data.'):'WAQI_TOKEN is missing in environment.');
    if (s.ok) o.style.color = 'var(--good)'; else o.style.color = 'var(--vpoor)';
    b.disabled=false;
  }
  $('#checkLive')?.addEventListener('click', runDiag);
