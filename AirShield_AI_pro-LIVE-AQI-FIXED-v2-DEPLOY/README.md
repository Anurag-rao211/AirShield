...
`js/api/waqi.js` holds the integration:

```js
const WAQI_TOKEN = "..."; // your token from https://aqicn.org/data-platform/token
```

When **Settings → Data mode → API mode** is selected, AirShield fetches real AQI,
pollutant and weather readings for the current city from `api.waqi.info` and merges
them into the dashboard. If a city has no nearby WAQI station, or the request fails,
AirShield falls back to its demo data automatically and shows a toast — it never
breaks the UI.
